...
4. Looks for a workbench installation at these paths:
    ...
    SD0:
    SD0:Workbench
    SD0:BOOT/$version/SYS (where $version is 13, 204, 31, 32 etc.)
...
2) Create a single <4GB Primary Fat32 partition 
   (smaller partitions may mount more quickly)
...
* If installing multiple workbench versions from Amiga, create the drawers listed under "Installing Multiple Workbenches" and install to the appropriate drawer.
...
- Confirming sufficient RAM on 1.3 is not trivial, and insufficient RAM will lead to a crash.
